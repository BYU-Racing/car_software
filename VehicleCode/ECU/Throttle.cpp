...
Throttle::Throttle(int pot1, int pot2) {
    this->potPin1 = pot1;
    this->potPin2 = pot2;
    magiMemory[0] = 0;
    magiMemory[1] = 0;
...
void Throttle::setThrottle1(int input) {
    this->throttle1 = map(input, MIN_THROTTLE_READ_POS, MAX_THROTTLE_READ_POS, MIN_THROTTLE_OUTPUT, MAX_THROTTLE_OUTPUT);
}

void Throttle::setThrottle2(int input) {
    this->throttle2 = map(-input, -MAX_THROTTLE_READ_NEG, -MIN_THROTTLE_READ_NEG, MIN_THROTTLE_OUTPUT, MAX_THROTTLE_OUTPUT);
}
